...
def generate_GOF(orth_vec, level):
    if level==2:
        target_norms = torch.tensor([num_of_data_in_parent_lv2[j] for j in range(4)]).float().cuda()
        GOF = orth_vec.T * target_norms.cuda()
    if level==3:
        target_norms = torch.tensor([num_of_data_in_parent_lv3[j] for j in range(118)]).float().cuda()
        GOF = orth_vec.T * target_norms.cuda()
    return GOF.T
...
np.savetxt('layer_3.txt', layer_3, fmt='%.6f')  
np.savetxt('layer_2.txt', layer_2, fmt='%.6f')  
...
#             print(j)
        ...
        if 'origin_logits_l5' in outputs:
...
#         print("Before losses dictionary:", losses)
#         l_dict = self.loss_proxy(outputs, targets, indices, num_boxes)
        
        
#         print("l_dict",l_dict)
#         losses.update(l_dict)
#         print("Final losses dictionary:", losses)
